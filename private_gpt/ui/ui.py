        ...
        def yield_deltas(completion_gen: CompletionGen) -> Iterable[str]:
            ...
            if completion_gen.sources:
                ...
                for source in cur_sources:
                    source_id = f"{source.file}-{source.page}"
                    if source_id not in used_files:
                        sources_text += f"* {source.file} | Page/Chunk: {source.page}\n"
                        used_files.add(source_id)
                full_response += sources_text
            elif mode == Modes.RAG_MODE:
